...
def goto_the_wanted_page(req):
    if "category[]" in req:
        splited_request = req.split(" ")[
            1
        ]  # look like: "/?category[]=auto&category[]=trains"
        desired_category = splited_request.split(
            "category[]="
        )  # look like: "auto&category[]"
        desired_category = [category[:-1] for category in desired_category[1:-1]] + [
            desired_category[-1]
        ]
        ...
        for categories in data_from_csv:
            if any(category in categories[2:] for category in desired_category):
                ...
                if categories[1] == "0":
                    data_from_csv_showed.append(
                        data_from_csv.pop(data_from_csv.index(categories))
                    )
                break
                ...
                if random_category[1] == "0":
                    data_from_csv_showed.append(
                        data_from_csv.pop(data_from_csv.index(random_category))
                    )
